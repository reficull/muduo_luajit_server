...
WebSocketFrameType WebSocket::parseHandshake(const char* input_frame, int input_len)
{
	// 1. copy char*/len into string
	// 2. try to parse headers until \r\n occurs
	string headers(const_cast<char*>(input_frame), input_len); 
	long unsigned int header_end = headers.find("\r\n\r\n");
	...
	int hRow = static_cast<int>(headers_rows.size());
	...
	for(int i=0; i<hRow; i++) {
		...
		if(header.find("GET") == 0) {
			...
			long unsigned int pos = header.find(":");
			if(pos != string::npos) {
				...
				else if(header_key == "Upgrade") this->upgrade = header_value;
			}
		}
	}
    ...
    if(this->key.length()>0 && this->upgrade == "websocket")
    {
        this->isRequest = true;
    }else{
    ...
    bool    theIncludeEmptyStrings)
{
	...
	long unsigned int  start = 0, end = 0, length = 0;
...
{
    unsigned digest[20]; // 160 bit sha1 digest
	...
	if(this->key.length() > 0) {
		...
		SHA1 sha;
		sha.Input(accept_key.data(), static_cast<unsigned int>(accept_key.size()));
		//unsigned char* dip = digest;
		sha.Result(digest);
		...
		for(int i=0; i<20; i+=4) {
			...
			c = static_cast<unsigned char>(digest[i]);
			digest[i] = digest[i+3];
			digest[i+3] = c;

			c = static_cast<unsigned char>(digest[i+1]);
		...
		accept_key = base64_encode(reinterpret_cast<const unsigned char *>(digest), 20); //160bit = 20 bytes/chars
...
int WebSocket::makeFrame(WebSocketFrameType frame_type, unsigned char* msg, int msg_length, unsigned char* buffer, int buffer_size)
{
	int pos = 0;
	int size = msg_length; 
	buffer[pos++] = static_cast<unsigned char>(frame_type); // text frame

	if(size<=125) {
		buffer[pos++] = static_cast<unsigned char>(size);
	}
	else if(size<=65535) {
		buffer[pos++] = 126; //16 bit length
		buffer[pos++] = static_cast<unsigned char>((size >> 8) & 0xFF); // rightmost first
		buffer[pos++] = static_cast<unsigned char>(size & 0xFF);
	}
	else { // >2^16-1
    ...
    memcpy(static_cast<void*>(buffer+pos), msg, size);
	return (size+pos);
}
...
{
	printf("getTextFrame()\n");
    std::cout << "in buffer:" << in_buffer << std::endl;
	...
	if(msg_masked) {
		mask = *(reinterpret_cast<unsigned int*>(in_buffer+pos));
		//printf("MASK: %08x\n", mask);
		pos += 4;
    ...
    std::cout << "payload:" << in_buffer << std::endl;
    memcpy(static_cast<void*>(out_buffer), static_cast<void*>(in_buffer+pos), payload_length);
	...
	*out_length = payload_length+1;
	...
	if(msg_opcode == 0x8) return DISCONNECT_FRAME;
...
extern "C" {
    ...
    void ws_gc(WebSocket *this_){
        delete this_;
    }

	WebSocketFrameType ws_parseHandshake(WebSocket* this_,const char* input_frame, int input_len){
        return this_->parseHandshake(input_frame,input_len);
    }

	const char* ws_answerHandshake(WebSocket* this_){
        return this_->answerHandshake().c_str();
    }

	int ws_makeFrame(WebSocket* this_,WebSocketFrameType frame_type, unsigned char* msg, int msg_len, unsigned char* buffer, int buffer_len){
        return this_->makeFrame(frame_type,msg,msg_len,buffer,buffer_len);
    }

	WebSocketFrameType ws_getFrame(WebSocket* this_, unsigned char* in_buffer, int in_length, unsigned char* out_buffer, int out_size, int* out_length){
    return this_->getFrame(in_buffer,in_length,out_buffer,out_size,out_length); 
    }
}
