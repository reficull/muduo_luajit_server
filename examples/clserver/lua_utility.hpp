...
///////////////////////////////////////////////////////////////////////
    ...
    static void call(lua_State* L, char const* funcname, Args const&... args)
    {
        ...
        std::cout << "lua utility call lua func :" << funcname << std::endl;
    ...
    template<typename Ret, typename T1, typename T2, typename T3, typename T4, typename T5,typename T6> // Return 1 value with 6 args
    static Ret call_R(lua_State* L, char const* funcname, T1 a1, T2 a2, T3 a3, T4 a4, T5 a5,T6 a6)
    {
        lua_getglobal(L, funcname); //tell lua to push the function into stack
    ...
    template<typename T1, typename T2, typename T3> // no return value with 3 args
    ...
    template<typename T1, typename T2, typename T3, typename T4, typename T5,typename T6> // no return value with 6 args
    static void call(lua_State* L, char const* funcname, T1 a1, T2 a2, T3 a3, T4 a4, T5 a5,T6 a6)
    {
        lua_getglobal(L, funcname); //tell lua to push the function into stack
    ...
    // working:static std::vector<std::string> getRet(lua_State* L,char const*  funcname,char const* a1,int a2,char const* a3){
    template<typename T2 ,typename T3,typename T4 > // get return vector with 4 args
    static std::vector<std::string> getRet(lua_State* L,char const*  funcname,T2 a1,T3 a2,T4 a3){
        ...
        lua_pushnil(L);
        while(lua_next(L, -2)) {
            char* tcstr = (char*)lua_tostring(L, -1);
            std::string var;
            if ((tcstr != NULL) && (tcstr[0] == '\0')) {
                printf("c is empty\n");
                var = std::string("");
                std::cout << "lua utility hpp get empty var" << std::endl;
            }else{
                var = std::string(tcstr);
                std::cout << "lua utility hpp get var:" << var << std::endl;
            }
            v.push_back(var);
            lua_pop(L, 1);
        }
...
template<> 
inline bool        Lua::fetch_(lua_State* L, int n) {
    return static_cast<bool>(lua_isboolean(L, n) ? lua_toboolean(L, n) : luaL_checkint(L, n));
}
template<> 
inline double      Lua::fetch_(lua_State* L, int n) { return luaL_checknumber(L, n); }
template<> 
inline    int      Lua::fetch_(lua_State* L, int n) { return luaL_checkint(L, n); }
template<> 
inline char const* Lua::fetch_(lua_State* L, int n) { return luaL_checkstring(L, n); }
