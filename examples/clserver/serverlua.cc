...
#define CONFIGFILE "jogconfig.cfg"
...
class EchoServer
{
            ...
            startTime_(Timestamp::now())
    {
        ...
        void start()
        {
            ...
            try
            {
                cfg.lookupValue("mongons",mongons);
                cout << "mongo ns: " << mongons<< endl << endl;
                cfg.lookupValue("mongouser",mongouser);
                //cout << "mongo user: " << mongouser << endl << endl;
                cfg.lookupValue("mongopasswd",mongopasswd);
                //cout << "mongo passwd: " << mongopasswd << endl << endl;
            }
                ...
                case 10000://request fight,v 是被挑戰者userId
                    {
                        ...
                        double RoomId = getMid("Room");//獲得計算出的最大id
                        ...
                        char msg2[200] = {0};
                        char msg1[200] = {0};
                        sprintf(msg2,"{protocol:10001,result: [0,%f,%d,%s,\"%s\",%d]}",RoomId,100,ids[0].c_str(),nickName.c_str(),image);
                        std::string msg2Str = std::string(msg2);
                        sprintf(msg1,"{protocol:10002,result: [0,%f,%d,%s]}",RoomId,100,ids[1].c_str());
